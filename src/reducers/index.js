...
export function isPromoApplied(state) {
  return state.cart.promoCodeHandler.promoApplied
}

export function getDiscountTotal(state){
    return state.cart.promoCodeHandler.discountTotal
}
...
export default function root(state, action) {
  if (action.type === ADD_TO_CART && state.products.byId[action.productId].quantity <= 0) return state
